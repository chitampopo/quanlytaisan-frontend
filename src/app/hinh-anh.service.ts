...
export class HinhAnhService {
  ...
  cleanFolder(folder: string, id: string): Observable<void> {
    let params = new HttpParams().set('folder', folder).set('id', id);
    return this.http.delete<void>(`/api/images/clean-folder`, { params });
  }

  uploadHinhAnhKhac(file: File, folder: string, id: string): Observable<void> {
    let params = new HttpParams().set('folder', folder).set('id', id);
  ...
  uploadHinhAnhDaiDien(file: File, folder: string, id: string): Observable<void> {
    let params = new HttpParams().set('folder', folder).set('id', id);
    let body = new FormData();
    body.append("file", file);
